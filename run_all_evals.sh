...
def upload_to_private_repo(
    repo_id: str, folder_path: str, hf_token: str, commit_message: str = "Model update"
) -> Optional[str]:
...
def make_hf_repo_public(repo_id: str, hf_token: str) -> bool:
    ...
    try:
        ...
        update_repo_settings(repo_id=repo_id, private=False, token=hf_token)
...
async def pull_command(uid: int, model_path: str, hf_token: Optional[str] = None):
    ...
    try:
        ...
        if hotkey not in commits:
            ...
            print(
                json.dumps(
                    {"success": False, "error": f"No commit found for UID {uid}"}
                )
            )
            sys.exit(1)
...
chute = build_sglang_chute(
    ...
    readme="{repo}",
    model_name="{repo}",
    image="chutes/sglang:nightly-2026032100",
    concurrency=40,
    revision="{revision}",
    ...
    max_instances=1,
    shutdown_after_seconds=73021,
        engine_args=(
            "--tool-call-parser qwen25 --mem-fraction-static 0.8 --chunked-prefill-size 4096 --context-length 65536 --max-completion-tokens 4096"
        ),
)
"""
    )
            ...
            if stats_data and stats_data.get("sampling_stats"):
                ...
                for window in [
                    ...
                    if window in sampling_stats:
                        ...
                        print(
                            f"  Samples/min: {wstats.get('samples_per_min', 0.0):.2f}"
                        )
                        print(
                            f"  Rate limit errors: {wstats.get('rate_limit_errors', 0)}"
                        )
                    ...
                    for env_name, env_data in env_stats.items():
                        ...
                        for window in [
                            ...
                            if window in env_data:
                                ...
                                print(f"    Success: {wstats.get('success', 0)}")
                                print(
                                    f"    Success rate: {wstats.get('success_rate', 0.0):.2%}"
                                )
                                print(
                                    f"    Samples/min: {wstats.get('samples_per_min', 0.0):.2f}"
                                )
                                ...
                                print(
                                    f"    Timeout errors: {wstats.get('timeout_errors', 0)}"
                                )
                                print(
                                    f"    Other errors: {wstats.get('other_errors', 0)}"
                                )
            else:
                print("\n" + "=" * 80)
    ...
    async with cli_api_client() as client:
        ...
        if data:
            ...
            if full:
                ...
                summary["sampled_task_ids"] = format_task_ids(
                    data.get("sampled_task_ids", [])
                )
                summary["pool_task_ids"] = format_task_ids(
                    data.get("pool_task_ids", [])
                )
                summary["missing_task_ids"] = format_task_ids(
                    data.get("missing_task_ids", [])
                )
...
async def deploy_command(
    ...
    if not skip_upload and not model_path:
        ...
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "--model-path is required unless --skip-upload is set",
                }
            )
        )
        sys.exit(1)
    ...
    if skip_upload and not revision:
        ...
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "--revision is required when --skip-upload is set",
                }
            )
        )
        sys.exit(1)
    ...
    if skip_chutes and not chute_id:
        ...
        print(
            json.dumps(
                {
                    "success": False,
                    "error": "--chute-id is required when --skip-chutes is set",
                }
            )
        )
        sys.exit(1)
    ...
    if not dry_run:
        ...
        if not skip_chutes:
            if not chutes_api_key:
                ...
                print(
                    json.dumps(
                        {"success": False, "error": "CHUTES_API_KEY not configured"}
                    )
                )
                sys.exit(1)
            ...
            if not chute_user:
                logger.error("CHUTE_USER not configured")
                print(
                    json.dumps({"success": False, "error": "CHUTE_USER not configured"})
                )
                sys.exit(1)
    ...
    if not skip_upload:
        ...
        logger.info(
            f"[{current_step}/{total_steps}] Uploading to HuggingFace ({repo}) [{upload_mode}]..."
        )

        if dry_run:
            logger.info(
                f"  [DRY RUN] Would upload {model_path} to {repo} (private={private_repo})"
            )
            ...
            try:
                ...
                if private_repo:
                    ...
                    logger.info(
                        f"  Private upload complete. Revision: {revision[:12]}..."
                    )
                else:
                    ...
                    api.upload_folder(
                        folder_path=model_path, repo_id=repo, commit_message=message
                    )
                ...
                print(
                    json.dumps(
                        {
                            "success": False,
                            "error": f"HuggingFace upload failed: {str(e)}",
                        }
                    )
                )
                sys.exit(1)
    ...
    if private_repo:
        ...
        if not skip_commit:
            current_step += 1
            logger.info(
                f"[{current_step}/{total_steps}] Committing on-chain (before deploy)..."
            )

            if dry_run:
                logger.info(
                    f"  [DRY RUN] Would commit {repo}@{revision[:12]}... with chute {chute_id}"
                )
            else:
                try:
                    import bittensor as bt
                    ...
                    sub = await get_subtensor()
                    data = json.dumps(
                        {"model": repo, "revision": revision, "chute_id": chute_id}
                    )
                    ...
                    while True:
                        try:
                            ...
                            if "SpaceLimitExceeded" in str(e):
                                logger.warning(
                                    "Space limit exceeded, waiting for next block..."
                                )
                                await sub.wait_for_block()
                            else:
                                raise
                    ...
                    print(
                        json.dumps(
                            {
                                "success": False,
                                "error": f"On-chain commit failed: {str(e)}",
                            }
                        )
                    )
                    sys.exit(1)
            ...
            logger.warning(
                "Private repo workflow: HF repo remains PRIVATE since commit was skipped"
            )
        ...
        if "make_public" in steps:
            ...
            if dry_run:
                ...
                if not make_hf_repo_public(repo, hf_token):
                    logger.warning(
                        "Failed to make repo public - manual intervention may be required"
                    )
                    logger.warning(
                        f"Run: huggingface-cli repo visibility {repo} --public"
                    )
                else:
...
"""
                    ...
                    if proc.returncode != 0:
                        logger.debug(
                            f"Chutes deploy failed with code {proc.returncode}"
                        )
                        raise RuntimeError("Chutes deploy failed")
                    ...
                    print(
                        json.dumps(
                            {
                                "success": False,
                                "error": f"Chutes deployment failed: {str(e)}",
                            }
                        )
                    )
                    sys.exit(1)
...
"""
    ...
    logger.info(f"  Repository: {repo}")
    logger.info(
        f"  Revision: {revision[:12] if revision and len(revision) > 12 else revision}..."
    )
    logger.info(f"  Chute ID: {chute_id}")
    logger.info("=" * 60)
    ...
    result = {
        ...
        if (private_repo and not skip_commit and not dry_run)
        else ("private" if private_repo else "public"),
    }
    print(json.dumps(result))
